...
export interface CotizacionDocumentosPanelProps {
  cotizacionId: string
...
/**
 * Panel de documentos de una cotizacion. Lista los documentos (tipo + enlace
 * externo + fecha) y, si el rol puede editar, permite subir archivos reales a
 * SharePoint via POST /api/je-admin/documentos/upload (entidadTipo "cotizacion")
 * y borrarlos via la server action eliminarDocumento. Refresca la ruta al exito.
 */
export function CotizacionDocumentosPanel({
  cotizacionId,
  documentos,
  puedeEditar,
}: CotizacionDocumentosPanelProps) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>): Promise<void> {
    ...
    formData.append("entidadTipo", "cotizacion")
    formData.append("entidadId", cotizacionId)
    ...
    <div className="flex flex-col gap-4">
      ...
      {documentos.length === 0 ? (
        <EmptyState
          ...
          description="Esta cotización no tiene documentos cargados."
          size="sm"
        />
      ) : (
